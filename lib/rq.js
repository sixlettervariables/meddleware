...
var RQ = module.exports = (function () {
    'use strict';
...
// and a requestion that will take the next step.
        ...
        series: function series(requestors, milliseconds) {

// RQ.series takes an array of requestor functions, and returns a requestor
// that will call them each in order. An initial value is passed to each requestor.

// If any of the requestor functions fail, then the whole sequence fails, and
// the remaining requestors are not called.

            check("RQ.series", requestors, milliseconds);
            return function requestor(requestion, initial) {
                var cancel,
                    timeout_id;
                ...
                check_requestion("RQ.series", requestion, initial);
                if (milliseconds) {
                    timeout_id = setTimeout(function () {
                        timeout_id = null;
                        return quash(expired("RQ.series", milliseconds));
                    }, milliseconds);
                }
                (function next(index) {
                    var requestor, r = requestion;
